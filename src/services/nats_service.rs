...
async fn create_users_topic() {
    ...
    let nats_conn = get_nats_connection().await.expect("Get Nats Connection Fail");
    match nats_conn.queue_subscribe(topic, queue) {
        Err(e) => error!("[NATS] Create queue subscriber for topic `{}` queue `{}` fail | {}", topic, queue, e),
        Ok(sub) => {
            info!("Subscribe topic `{}` queue `{}` success", topic, queue);
            sub.with_handler(move |msg| {
                let nats_req = NatsRequest::from(msg.clone());
                info!("[IN] topic `{}` queue `{}`: message: {:?}", topic, queue, nats_req);
                msg.respond("")
            });
        }
